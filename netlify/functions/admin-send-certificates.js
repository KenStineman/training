  ...
  try {
    ...
    for (const cert of certificates) {
      try {
        ...
        const emailSent = await sendCertificateEmail(cert);
        ...
        console.error(`Failed to send email for certificate ${cert.id}:`, err);
        errors.push({ certificateId: cert.id, error: err.message });
      }
    }
...
async function sendCertificateEmail(cert) {
